...
## Quality control and corroboration
...
For ProtNLM2 predictions, we have reduced the model score threshold to 0.05 as many predictions with low model scores were judged to be accurate. For predictions above this threshold, Google DeepMind have implemented an additional post-processing strategy (described below) to filter out unreliable predictions. Based on ongoing curator evaluation and user feedback on the ProtNLM2 pilot release, this model score threshold may be altered (increased or removed) for future ProtNLM2 releases. Model scores for all predictions are provided for the user to make their own assessment on prediction reliability.

### The Evidencer: prediction evaluation

To ensure the reliability of the data presented to users, ProtNLM2 predictions above the model score threshold are further evaluated using a post-processing system known as the *Evidencer*. This pipeline attempts to emulate expert [biocurator evaluation](https://www.uniprot.org/help/biocuration) by filtering out unreliable predictions and requiring corroborating evidence before a prediction is displayed. It consists of two parts:

*1. Exclusion Evidencer*

Predictions are rejected if they match exclusion criteria such as GO annotations that are restricted by [GO taxon constraints](https://geneontology.org/docs/taxon-constraints/#:~:text=Taxon%20constraints%20are%20used%20to,specificity%20of%20the%20GO%20terms), or inaccurate protein names that do not conform to the [International Protein Nomenclature Guidelines](https://www.uniprot.org/help/international_protein_nomenclature_guidelines).

*2. Corroboration Evidencer*

Predicted annotations are corroborated by:
...
* **Structural Similarity:** If there is no corroboration by sequence similarity, the process checks structural similarity (determined by the [tm-align tool](https://www.rcsb.org/alignment)) to other entries in UniProtKB containing annotations identical to the predicted annotation. Corroboration requires the TM-align score normalized to the protein with the predicted annotation to be greater than 0.5 and the TM-align score normalized to the corroborating protein possessing a matching prediction to be greater than 0.1. Additionally, structural similarity corroboration requires a minimum sequence identity of 5%, a minimum of 3 matching secondary structure elements and a minimum pLDDT score for aligned residues of 70:
  ...
  |                                            |        |
  |:-------------------------------------------|--------|
  | TM-align score (protein with prediction) | >= 0.5 |
  | TM-align score (protein with matching annotation) | >= 0.1  |
  | Sequence identity | >= 5% |
  | Matching secondary structure elements | >= 3 |
  | pLDDT of aligned residues | >= 70  |

The *Evidencer* workflow produces three possible outcomes:

* **Rejected**: Predictions rejected by the *Exclusion Evidencer* are not displayed in UniProtKB.
* **Approved**: Predictions corroborated by the *Corroboration Evidencer* are displayed in UniProtKB.
* **Uncertain**: Predictions that are neither rejected nor corroborated have uncertain quality and are not displayed in UniProtKB.
...
Predictions are excluded if their ProtNLM model score is below 0.05, or if they are excluded by the *Exclusion Evidencer* or if they cannot be corroborated by the *Corroboration Evidencer*.
