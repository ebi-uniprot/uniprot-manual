...
Entries with variant annotations can be retrieved on the UniProt website with the query [`annotation:(type:variant)`](http://www.uniprot.org/uniprotkb?query=annotation:%28type:variant%29) .
...
We have phased out support for Microsoft's Internet Explorer (IE 11), since our user base for this web browser is continuously decreasing and has significantly fallen below 1.5%. Please switch to a more recent and [supported browser](http://www.uniprot.org/help/browser_support) in order to enjoy the full functionalities of the UniProt website.
