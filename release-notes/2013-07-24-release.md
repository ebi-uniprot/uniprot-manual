...
UniProtKB/Swiss-Prot is currently linked to this resource from the cross-reference section (DR lines), but we also have some site-specific links from the sequence annotation section (FT_CARBOHYD) of [relevant UniProtKB/Swiss-Prot entries](https://www.uniprot.org/uniprotkb?query=annotation%3A%28type%3Acarbohyd+CAR_*%29). An increase of the number of cross-linked entries is planned, including more literature based glycan data from UniCarbKB.
